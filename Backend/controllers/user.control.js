...
export const followUnfollowUser = async(req,res)=>{
    try {
        ...
        if(isFollowing){
            ...
            return res.status(400).json({message: "Unfollow successfully"})
        }
        else{
            //follow
            ...
            return res.status(400).json({message: "follow successfully"})
...
export const updateUser = async(req,res) =>{
    try {
        ...
        if(currentPassword && newPassword){
            const isMatch = await bcrypt.compare(currentPassword, user.password)//password we stored in the mongoDB
        
        if(!isMatch){
            return res.status(400).json({error: "Current Password is Incorrect"})
        }
        if(newPassword.lemgth<6){
